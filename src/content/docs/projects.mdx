...
:::note
`owner` is the email of the project owner (the creator). The owner never appears in the project's collaborator list — use [Get Project Collaborators](#get-project-collaborators) to list everyone else who has access.
:::
...
:::note
If a user or group is granted multiple roles, the entry carries the highest-priority one, where `PROJECT_ADMIN` > `PROJECT_COLLABORATOR` > `PROJECT_VIEWER`. Inactive users are excluded, the owner never appears in the list, and the order of entries is not guaranteed.
:::
...
:::note
Inactive users are excluded from `members`. Built-in system groups are not accessible through this endpoint.
:::
